...
const OrderCard: FC<OrderCardProps> = ({ sale, money, onAdvance, onCancel, canAct }) => {
...
export const LiveOrdersPage: FC = () => {
  ...
  return (
    <AdminPage
      title="Live Orders"
      subtitle={`${active.length} order${active.length === 1 ? '' : 's'} in flight - updates in real time.`}
    ...
    >
      <div className={cls.kanban}>
        {ACTIVE_ORDER_STATUSES.map((status) => {
          ...
          return (
            <section key={status} className={cls.kanbanCol} aria-label={meta.title}>
